...
export function ProfileCard() {
  ...
  return (
    <div className="rise relative w-full overflow-hidden rounded-[2rem] bg-ink px-5 py-4 text-ink-foreground shadow-panel sm:px-6 sm:py-5">
      {/* portrait */}
      <div className="relative overflow-hidden rounded-[1.4rem] bg-primary">
        <img
          src={profileImg}
          alt={`${profile.name}, ${profile.role}`}
          width={912}
          height={1104}
          className="h-[15rem] w-full object-cover object-[50%_18%] mix-blend-luminosity opacity-90 xl:h-[17rem]"
        />
      ...
      {/* name + one-liner, centred like the reference */}
      <div className="mt-4 text-center">
        <h1 className="text-[1.85rem] font-bold leading-none tracking-[-0.03em] text-ink-foreground">
          {profile.name}
        </h1>
        <p className="mt-2.5 text-[0.78rem] font-medium uppercase tracking-[0.14em] text-ink-foreground/45">
          {profile.role}
        </p>
      </div>

      <div className="mx-auto mt-3 grid h-8 w-8 place-items-center rounded-full bg-primary text-primary-foreground">
        <span className="text-xs font-bold">KA</span>
      </div>

      <p className="mx-auto mt-3 max-w-[15rem] text-center text-[0.78rem] leading-relaxed text-ink-foreground/55">
        {profile.intro}
      </p>

      <div className="mt-2 flex items-center justify-center gap-1.5 text-[0.65rem] text-ink-foreground/40">
      ...
      <div className="mt-6 flex flex-col gap-2.5 border-t border-ink-foreground/10 pt-5">
        <a
